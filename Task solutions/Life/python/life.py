...
def keep_within_bounds(i) : return  i - size if i >= size else  i + size if i < 0 else i

def live_neighbour_count(grid, cellNo) : return len(list(filter(lambda relPos: grid[keep_within_bounds(cellNo + relPos)], relative_neighbour_positions)))
